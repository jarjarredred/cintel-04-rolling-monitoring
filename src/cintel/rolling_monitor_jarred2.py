...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "rolling_metrics_jarred2.csv"
...
def main() -> None:
    ...
    # ----------------------------------------------------
    # STEP 3.3.5: DEFINE ROLLING ERROR RATE (%)
    # ----------------------------------------------------
    # (Rolling Errors / Rolling Requests) * 100
    error_rate_recipe: pl.Expr = (
        (
            pl.col("errors").rolling_sum(WINDOW_SIZE)
            / pl.col("requests").rolling_sum(WINDOW_SIZE)
        )
        * 100
